...
def generate_comprehensive_pdf(session_key: str) -> bytes:
    ...
    try:
        ...
        if session.overall_performance_feedback or session.overall_performance_score:
            pdf.add_page()
            pdf.set_font("Arial", "B", 14)
            pdf.cell(0, 8, _sanitize_for_pdf("Overall Evaluation"), ln=True)
            pdf.ln(3)
            
            if session.overall_performance_score:
                pdf.set_font("Arial", "B", 12)
                pdf.cell(0, 7, _sanitize_for_pdf(f"Overall Score: {session.overall_performance_score:.1f}/100"), ln=True)
                pdf.ln(2)
