    ...
    try:
        ...
        if response.status_code == 201:
            ...
            if response.status_code == 200:
                ...
                for job in jobs:
                    if (
                        job.get("job_title") == data["job_title"]
                        and job.get("company_name") == data["company_name"]
                    ):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for i, slot in enumerate(slots[:5]):  # Show first 5 slots
                ...
                log(
                    f"     Available: {slot.get('current_bookings', 0)}/{slot.get('max_candidates', 1)}"
                )
...
def test_get_available_slots(token):
    ...
    try:
        response = requests.get(
            f"{BASE_URL}/api/interviews/available-slots/", headers=headers
        )
...
def test_search_available_slots(token):
    ...
    try:
        response = requests.post(
            f"{BASE_URL}/api/interviews/slots/search_available/",
            json=data,
            headers=headers,
        )
        ...
        if response.status_code == 200:
            ...
            log(
                f"✅ Search found {len(available_slots)} available slots for {search_date}"
            )
    ...
    try:
        ...
        if response.status_code == 201:
            ...
            if created_count > 0:
                ...
                log(
                    f"⚠️ Created 0 recurring slots (conflicts detected - this is correct behavior)"
                )
...
def test_slot_availability_status(token, slot_id):
    ...
    try:
        response = requests.get(
            f"{BASE_URL}/api/interviews/slots/{slot_id}/", headers=headers
        )
        ...
        if response.status_code == 200:
            ...
            is_available = slot.get("current_bookings", 0) < slot.get(
                "max_candidates", 1
            )
...
def test_release_slot(token, slot_id):
    ...
    try:
        response = requests.post(
            f"{BASE_URL}/api/interviews/slots/{slot_id}/release_slot/", headers=headers
        )
...
def test_calendar_view(token):
    ...
    try:
        response = requests.get(
            f"{BASE_URL}/api/interviews/calendar/", params=params, headers=headers
        )
...
def main():
    ...
    log(
        f"   ✅ Slot release: {'PASS' if 'release_result' in locals() and release_result else 'FAIL'}"
    )
    ...
    test_results = [
        ...
        slot_details,
        booking_result,
        "release_result" in locals() and release_result,
        calendar_result,
    ]
    ...
    if passed_tests == total_tests:
        log(
            "🎉 All tests passed! Interview slot management system is working correctly."
        )
    else:
