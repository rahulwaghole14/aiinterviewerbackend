...
class FileUploadAPITester:
    ...
    def test_bulk_candidate_extract(self):
        ...
        if os.path.exists(test_files_dir):
            for filename in os.listdir(test_files_dir):
                if filename.endswith('.pdf'):
                    ...
                    resume_files.append(('resume_files', (filename, open(file_path, 'rb'), 'application/pdf')))
        ...
        try:
            ...
            self.log_test("/candidates/bulk-create/?step=extract", "POST", response.status_code,
            ...
            for _, (_, file_obj, _) in resume_files:
                file_obj.close()
            self.log_test("/candidates/bulk-create/?step=extract", "POST", 0, error=str(e))
            return False
    ...
    def test_bulk_candidate_submit(self):
        ...
        self.log_test("/candidates/bulk-create/?step=submit", "POST", response.status_code,
                     response.json() if response.status_code < 300 else None,
                     response.text if response.status_code >= 400 else None)
    ...
    def test_create_candidate_with_resume(self):
        ...
        if os.path.exists(test_files_dir):
            for filename in os.listdir(test_files_dir):
                if filename.endswith('.pdf'):
                    file_path = os.path.join(test_files_dir, filename)
                    resume_file = ('resume_file', (filename, open(file_path, 'rb'), 'application/pdf'))
                    break
        ...
        try:
            ...
            self.log_test("/candidates/", "POST", response.status_code,
                         response.json() if response.status_code < 300 else None,
                         response.text if response.status_code >= 400 else None)
    ...
    def test_bulk_resume_upload(self):
        ...
        if os.path.exists(test_files_dir):
            for filename in os.listdir(test_files_dir):
                if filename.endswith('.pdf'):
                    file_path = os.path.join(test_files_dir, filename)
                    resume_files.append(('files', (filename, open(file_path, 'rb'), 'application/pdf')))
        ...
        try:
            ...
            self.log_test("/resumes/bulk-upload/", "POST", response.status_code,
                         response.json() if response.status_code < 300 else None,
                         response.text if response.status_code >= 400 else None)
    ...
    def test_incorrect_methods(self):
        ...
        self.log_test("/candidates/bulk-create/?step=extract", "GET", response.status_code,
                     None, response.text if response.status_code >= 400 else None)
