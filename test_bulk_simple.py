#!/usr/bin/env python3
"""
Simple test script for Bulk Resume Upload API
...
def test_bulk_resume_upload():
    ...
    auth_data = {"username": "admin", "email": "admin@test.com", "password": "admin123"}

    auth_response = requests.post(f"{BASE_URL}/auth/login/", json=auth_data)
    if auth_response.status_code != 200:
        log("Failed to authenticate with admin. Trying without auth...", "WARNING")
        headers = {}
    else:
