...
def make_request(method, url, data=None, headers=None, expected_status=200):
    ...
    try:
        ...
        if response.status_code == expected_status:
            ...
            log(f"✗ {method} {url} - Status: {response.status_code}, Response: {response.text}", "ERROR")
            return None
...
def test_dashboard_endpoints():
    ...
    for user_type, user_data in test_users.items():
        ...
        summary_data = make_request("GET", f"{API_BASE}/dashboard/summary/", headers=headers)
        if summary_data:
        ...
        for endpoint in stats_endpoints:
            stats_data = make_request("GET", f"{API_BASE}/dashboard/{endpoint}", headers=headers)
            if stats_data:
                log(f"   {endpoint} data received")
        
        # 4. User Activities
        log("4. Testing user activities...")
        activities_data = make_request("GET", f"{API_BASE}/dashboard/activities/", headers=headers)
        if activities_data:
        ...
        # List widgets
        widgets_data = make_request("GET", f"{API_BASE}/dashboard/widgets/", headers=headers)
        if widgets_data:
        ...
        if user_type == "admin":
            test_widget = {
                ...
                "settings": {
                    "chart_type": "bar",
                    "data_source": "resume_stats"
                },
                "is_active": True
            }
            
            widget_response = make_request("POST", f"{API_BASE}/dashboard/widgets/", test_widget, headers=headers)
            if widget_response:
                ...
                update_response = make_request("PUT", f"{API_BASE}/dashboard/widgets/{widget_id}/", update_data, headers=headers)
                if update_response:
                    log(f"   Updated widget successfully")
                
                # Delete widget
                delete_response = make_request("DELETE", f"{API_BASE}/dashboard/widgets/{widget_id}/", headers=headers, expected_status=204)
                if delete_response is not None:
                    log(f"   Deleted widget successfully")
        
        # 6. Widget Settings
        log("6. Testing widget settings...")
        settings_data = make_request("GET", f"{API_BASE}/dashboard/widgets/settings/", headers=headers)
        if settings_data:
            log(f"   Widget settings received")
        
        # 7. Chart Data
        log("7. Testing chart data...")
        chart_types = ["resume_trends", "interview_performance", "candidate_distribution", "job_statistics"]
        
        for chart_type in chart_types:
            chart_data = make_request("GET", f"{API_BASE}/dashboard/charts/{chart_type}/", headers=headers)
            if chart_data:
    ...
    if test_users["admin"]["token"]:
        ...
        for filter_param in date_filters:
            filtered_data = make_request("GET", f"{API_BASE}/dashboard/{filter_param}", headers=headers)
            if filtered_data:
...
def test_dashboard_analytics():
    ...
    try:
        ...
        interview_stats = DashboardAnalytics._get_interview_stats(user, start_date, end_date, {})
        log(f"   Interview statistics: {interview_stats}")
        
        candidate_stats = DashboardAnalytics._get_candidate_stats(user, start_date, end_date, {})
        ...
        activity_data = DashboardAnalytics._get_activity_data(user, start_date, end_date)
        log(f"   User activity: {activity_data}")
        
        performance_data = DashboardAnalytics._get_performance_data(start_date, end_date)
...
def test_dashboard_models():
    ...
    try:
        ...
        from dashboard.models import DashboardMetric, UserActivity, DashboardWidget, SystemPerformance
        from django.contrib.auth import get_user_model
