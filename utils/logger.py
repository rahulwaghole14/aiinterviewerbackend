...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/ai_interviewer.log'),
...
class LogSanitizer:
    ...
    SENSITIVE_FIELDS = [
        ...
        'ssn', 'credit_card', 'bank_account', 'phone', 'address'
    ]
    ...
    def sanitize_data(data):
        ...
        if isinstance(data, dict):
            ...
            for key, value in data.items():
                if any(sensitive in key.lower() for sensitive in LogSanitizer.SENSITIVE_FIELDS):
                    sanitized[key] = '[REDACTED]'
                elif isinstance(value, dict):
                    sanitized[key] = LogSanitizer.sanitize_data(value)
                elif isinstance(value, list):
                    sanitized[key] = [LogSanitizer.sanitize_data(item) if isinstance(item, dict) else item for item in value]
                else:
                    sanitized[key] = value
            return sanitized
        return data
