...
from .serializers import (
    ...
    NotificationSummarySerializer, MarkNotificationReadSerializer,
    CreateNotificationSerializer, NotificationFilterSerializer,
...
class NotificationSummaryView(APIView):
    ...
    def get(self, request):
        ...
        # Counts by priority
        urgent_count = notifications.filter(priority=NotificationPriority.URGENT).count()
        high_count = notifications.filter(priority=NotificationPriority.HIGH).count()
        medium_count = notifications.filter(priority=NotificationPriority.MEDIUM).count()
        ...
        if serializer.is_valid():
            ...
            try:
                ...
                notification = NotificationService.create_notification(
                    ...
                    priority=serializer.validated_data.get('priority', NotificationPriority.MEDIUM),
...
class NotificationTemplateListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        # Check if user is admin
        if not (self.request.user.is_superuser or getattr(self.request.user, 'role', '').lower() == 'admin'):
