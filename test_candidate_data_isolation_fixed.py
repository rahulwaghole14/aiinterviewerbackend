...
def test_candidate_data_isolation():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(
                f"   📊 Candidates with recruiter: {len(candidates_by_recruiter)} recruiters"
            )
            print(
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if not agencies_with_candidates:
                ...
                print(
                    "   ℹ️  This might be because candidates were created by admin users"
                )
        else:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(
                f"   ✅ Candidates with recruiter (can be isolated): {candidates_with_company_info}"
            )
            print(
                f"   ⚠️  Candidates without recruiter (cannot be isolated): {candidates_without_company_info}"
            )
            ...
            if candidates_with_company_info > 0:
                ...
                print(
                    "   ⚠️  No candidates have recruiters assigned - data isolation won't work"
                )
        else:
            print(f"   ❌ Failed to get candidates: {response.text}")
    except Exception as e:
