...
from utils.hierarchy_permissions import (
    HiringAgencyHierarchyPermission,
    DataIsolationMixin,
)
...
class UserDataViewSet(DataIsolationMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        base_queryset = UserData.objects.filter(role="Hiring Agency").order_by(
            "-id"
        )  # Show newest first
    ...
    def update(self, request, *args, **kwargs):
        ...
        if not self.has_object_permission(request, self, instance):
            return Response(
                {
                    "detail": "You can only edit hiring agency data for your own company."
                },
                status=status.HTTP_403_FORBIDDEN,
            )
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        if not self.has_object_permission(request, self, instance):
            return Response(
                {
                    "detail": "You can only delete hiring agency data for your own company."
                },
                status=status.HTTP_403_FORBIDDEN,
            )
    ...
    def has_permission_to_create(self, creator_role, new_user_role):
        if creator_role == "SUPER_ADMIN":
            return True
        elif creator_role == "ADMIN" and new_user_role in [
            "HR",
            "HIRING_MANAGER",
            "INTERVIEWER",
            "OTHER",
            ...
            return True
        elif creator_role == "COMPANY" and new_user_role in [
            "HR",
            "HIRING_MANAGER",
            "INTERVIEWER",
            "OTHER",
