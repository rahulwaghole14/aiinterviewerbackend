...
def test_bulk_resume_upload_permissions(token, user_type):
    ...
    files = [
        (
            "files",
            ("test_resume1.pdf", io.BytesIO(mock_pdf_content), "application/pdf"),
        ),
        (
            "files",
            ("test_resume2.pdf", io.BytesIO(mock_pdf_content), "application/pdf"),
        ),
    ]
...
def test_interview_management_permissions(token, interview_id, user_type):
    ...
    try:
        ...
        print(
            f"📤 PUT /api/interviews/{interview_id}/ - Status: {response.status_code}"
        )
...
if __name__ == "__main__":
    ...
    if hiring_agency_token:
        ...
        interview_id = test_interview_scheduling_permissions(
            hiring_agency_token, "HIRING AGENCY"
        )
        if interview_id:
            test_interview_management_permissions(
                hiring_agency_token, interview_id, "HIRING AGENCY"
            )
    ...
    if recruiter_token:
        ...
        # Test interview scheduling
        interview_id = test_interview_scheduling_permissions(
            recruiter_token, "RECRUITER"
        )
        if interview_id:
            test_interview_management_permissions(
                recruiter_token, interview_id, "RECRUITER"
            )
    ...
    if company_token:
        ...
        if interview_id:
            test_interview_management_permissions(
                company_token, interview_id, "COMPANY"
            )
    ...
    print(
        "❓ Need to check if only hiring agency and recruiter can schedule interviews"
    )
