...
class CRUDTester:
    ...
    def test_recruiter_crud(self) -> Dict[str, Any]:
        ...
        try:
            response = self.session.post(
                f"{BASE_URL}/api/companies/recruiters/", json=recruiter_data
            )
            if response.status_code == 201:
    ...
    def test_candidate_crud(self) -> Dict[str, Any]:
        ...
        candidate_data = {
            "name": f"Test Candidate {int(time.time())}",
        ...
        try:
            response = self.session.post(
                f"{BASE_URL}/api/candidates/", json=candidate_data
            )
            if response.status_code == 201:
        ...
        if self.test_data.get("candidate_id"):
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    print(f"✅ Candidate detail retrieved: {candidate_detail['name']}")
                else:
                    print(
        ...
        if self.test_data.get("candidate_id"):
            ...
            update_data = {
                "name": f"Updated Candidate {int(time.time())}",
                "experience_years": 7,
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    print(
                        f"✅ Candidate updated successfully: {updated_candidate['name']}"
                    )
                    results["update"] = True
                else:
                    print(
