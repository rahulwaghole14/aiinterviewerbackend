        ...
        if response.status_code == 200:
            ...
            for i, agency in enumerate(hiring_agencies, 1):
                ...
                if agency.get("company") is None and agency.get("company_name"):
                    print(
                        f"   ⚠️  ISSUE DETECTED: company FK is null but company_name exists!"
                    )
                    print(
                        f"   📝 This suggests the ForeignKey relationship wasn't properly set during migration or creation."
                    )
                    print()
                elif agency.get("company") is None and not agency.get("company_name"):
                    print(
                        f"   ℹ️  No company assigned (both FK and text field are empty)"
                    )
                    print()
                else:
            ...
            agencies_with_null_company = [
                a for a in hiring_agencies if a.get("company") is None
            ]
            agencies_with_company = [
                a for a in hiring_agencies if a.get("company") is not None
            ]

            print(f"   Total agencies: {len(hiring_agencies)}")
            print(
                f"   Agencies with null company FK: {len(agencies_with_null_company)}"
            )
            print(f"   Agencies with company FK: {len(agencies_with_company)}")

            if agencies_with_null_company:
                print(
                    f"   ⚠️  {len(agencies_with_null_company)} agencies have null company field!"
                )
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for company in companies:
                print(
                    f"   ID: {company.get('id')} | Name: {company.get('name')} | Email: {company.get('email')}"
                )
        else:
