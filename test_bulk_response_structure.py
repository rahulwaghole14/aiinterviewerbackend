...
def test_bulk_upload_endpoint():
    ...
    try:
        response = requests.post(
            f"{API_BASE}/resumes/bulk-upload/", data={"invalid": "data"}
        )
        log(f"Response Status: {response.status_code}")
        log(f"Response: {response.text}")
    except Exception as e:
...
def show_expected_response_structure():
    ...
    error_response = {"files": ["This field is required."]}
    log(json.dumps(error_response, indent=2))

    log("\nExpected Authentication Error (401 Unauthorized):")
    auth_error = {"error": "Authentication credentials were not provided."}
