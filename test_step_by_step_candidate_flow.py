...
def test_step1_domain_role_selection(token):
    ...
    data = {"domain": "Python Development", "role": "Senior Software Engineer"}

    try:
        response = requests.post(
            f"{BASE_URL}/api/candidates/select-domain/", json=data, headers=headers
        )
...
def test_step2_data_extraction(token, draft_id):
    ...
    try:
        response = requests.post(
            f"{BASE_URL}/api/candidates/extract-data/",
            files=files,
            data=data,
            headers=headers,
        )
...
def test_step3_verification_preview(token, draft_id):
    ...
    try:
        response = requests.get(
            f"{BASE_URL}/api/candidates/verify/{draft_id}/", headers=headers
        )
...
def test_step4_verification_update(token, draft_id):
    ...
    try:
        response = requests.put(
            f"{BASE_URL}/api/candidates/verify/{draft_id}/", json=data, headers=headers
        )
...
def test_step5_final_submission(token, draft_id):
    ...
    try:
        response = requests.post(
            f"{BASE_URL}/api/candidates/submit/{draft_id}/", json=data, headers=headers
        )
