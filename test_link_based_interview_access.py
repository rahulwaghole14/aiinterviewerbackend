...
def create_test_candidate(token, company_id):
    ...
    try:
        response = requests.post(f"{BASE_URL}/api/candidates/", json=data, headers=headers)
        if response.status_code == 201:
            return response.json()['id']
        else:
            log(f"Candidate creation failed: {response.status_code} - {response.text}", "ERROR")
            return None
    except Exception as e:
...
def schedule_interview(token, candidate_id, job_id):
    ...
    try:
        response = requests.post(f"{BASE_URL}/api/interviews/", json=data, headers=headers)
        if response.status_code == 201:
            result = response.json()
            log(f"Interview scheduled successfully: {result['id']}")
            return result
        else:
            log(f"Interview scheduling failed: {response.status_code} - {response.text}", "ERROR")
            return None
    except Exception as e:
...
def generate_interview_link(token, interview_id):
    ...
    try:
        response = requests.post(f"{BASE_URL}/api/interviews/{interview_id}/generate-link/", headers=headers)
        if response.status_code == 200:
            result = response.json()
            log(f"Interview link generated: {result['interview_link'][:20]}...")
            return result
        else:
            log(f"Link generation failed: {response.status_code} - {response.text}", "ERROR")
            return None
    except Exception as e:
...
def test_public_interview_access(interview_link_token):
    ...
    try:
        response = requests.get(f"{BASE_URL}/api/interviews/public/{interview_link_token}/")
        ...
        if response.status_code == 200:
            ...
            log(f"   Scheduled: {data.get('scheduled_date')} at {data.get('scheduled_time')}")
...
def test_join_interview(interview_link_token):
    ...
    try:
        response = requests.post(f"{BASE_URL}/api/interviews/public/{interview_link_token}/")
...
def test_invalid_link():
    ...
    try:
        ...
        if response.status_code == 400:
            ...
            log(f"❌ Invalid link not properly handled: {response.status_code}", "ERROR")
            return False
    except Exception as e:
...
def test_expired_link():
    ...
    try:
        ...
        if response.status_code == 400:
            ...
            log(f"❌ Expired link not properly handled: {response.status_code}", "ERROR")
            return False
    except Exception as e:
...
def main():
    ...
    if all([success1, success2, success3, success4]):
        log("🎉 All tests passed! Link-based interview access system is working correctly.")
    else:
