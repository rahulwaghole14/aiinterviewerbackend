...
def test_hiring_agency_management(token, user_type):
    ...
    try:
        ...
        if response.status_code == 201:
            ...
            print(
                f"❌ {user_type} blocked from creating hiring agency users (403 Forbidden)"
            )
            return None
        else:
...
def test_hiring_agency_edit(token, user_id, user_type):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(
                f"❌ {user_type} blocked from editing hiring agency users (403 Forbidden)"
            )
        else:
            print(f"❌ Unexpected response for {user_type}: {response.status_code}")
    except Exception as e:
...
def test_hiring_agency_delete(token, user_id, user_type):
    ...
    try:
        response = requests.delete(
            f"{BASE_URL}/hiring_agency/{user_id}/", headers=headers
        )
        ...
        if response.status_code == 204:
            ...
            print(
                f"❌ {user_type} blocked from deleting hiring agency users (403 Forbidden)"
            )
        else:
            print(f"❌ Unexpected response for {user_type}: {response.status_code}")
    except Exception as e:
