...
def test_company_permissions(token, user_type):
    ...
    try:
        ...
        if response.status_code == 201:
            ...
            print(
                f"✅ {user_type} correctly blocked from creating companies (403 Forbidden)"
            )
            return None
        else:
...
def test_edit_permissions(token, company_id, user_type):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(
                f"✅ {user_type} correctly blocked from editing companies (403 Forbidden)"
            )
        else:
            print(f"❌ Unexpected response for {user_type}: {response.status_code}")
    except Exception as e:
...
def test_delete_permissions(token, company_id, user_type):
    ...
    try:
        ...
        if response.status_code == 204:
            ...
            print(
                f"✅ {user_type} correctly blocked from deleting companies (403 Forbidden)"
            )
        else:
            print(f"❌ Unexpected response for {user_type}: {response.status_code}")
    except Exception as e:
