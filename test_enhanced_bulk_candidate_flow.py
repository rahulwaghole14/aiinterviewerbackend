...
def test_enhanced_bulk_candidate_flow():
    ...
    if media_dir.exists():
        ...
        for pdf_file in pdf_files:
            test_files.append(('resume_files', (pdf_file.name, open(pdf_file, 'rb'), 'application/pdf')))
    ...
    if not test_files:
        ...
        response = requests.post(f"{API_BASE}/candidates/bulk-create/?step=extract", data=test_data, headers={'Authorization': f'Token {token}'})
    ...
    if response.status_code == 200:
        ...
        for i, candidate in enumerate(extracted_candidates, 1):
            if candidate.get('can_edit'):
                ...
                print(f"      Experience: {extracted_data.get('work_experience', 'Not found')} years")
            else:
                print(f"   {i}. ❌ {candidate.get('filename')} -> Error: {candidate.get('error')}")
        ...
        if response.status_code == 201:
            ...
            for i, res in enumerate(results, 1):
                if res.get('success'):
                    print(f"   {i}. ✅ {res.get('filename')} -> Candidate: {res.get('candidate_name')}")
                else:
                    print(f"   {i}. ❌ {res.get('filename')} -> Error: {res.get('error_message')}")
    ...
    response = requests.get(f"{API_BASE}/candidates/", headers={'Authorization': f'Token {token}'})
    ...
    if response.status_code == 200:
        ...
        for candidate in recent_candidates:
            print(f"   - {candidate.get('full_name', 'Unknown')} ({candidate.get('domain')}) - {candidate.get('status')}")
...
def test_enhanced_flow_validation():
    ...
    response = requests.post(f"{API_BASE}/candidates/bulk-create/?step=extract", data=test_data, headers=headers)
    ...
    response = requests.post(f"{API_BASE}/candidates/bulk-create/?step=submit", json=test_data, headers=headers)
    print(f"   Status: {response.status_code} (Expected: 400)")
    if response.status_code == 400:
        print("   ✅ Validation working")
