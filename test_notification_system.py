...
def test_notification_system():
    ...
    print(
        f"✅ Created users: {admin_user.email}, {company_user.email}, {recruiter_user.email}"
    )
    ...
    print(
        f"✅ User preferences: Email={preferences.email_enabled}, In-App={preferences.in_app_enabled}"
    )
    ...
    unread_count_before = NotificationService.get_unread_notifications_count(
        recruiter_user
    )
    ...
    unread_count_after = NotificationService.get_unread_notifications_count(
        recruiter_user
    )
    ...
    summary_data = {
        ...
        "failed_count": user_notifications.filter(
        ...
        "high_count": user_notifications.filter(
            priority=NotificationPriority.HIGH
        ).count(),
        "medium_count": user_notifications.filter(
            priority=NotificationPriority.MEDIUM
        ).count(),
        "low_count": user_notifications.filter(
    ...
    try:
        resume_notification = NotificationService.send_resume_processed_notification(
            resume=None, recipient=recruiter_user  # We don't have a real resume object
        )
        ...
        print(
            f"   ❌ Resume processed notification failed (expected - no resume object): {str(e)}"
        )

    # Test bulk upload notification
    print("   Testing bulk upload notification...")
    bulk_results = {"summary": {"total_files": 5, "successful": 4, "failed": 1}}
    ...
    try:
        ...
        print(
            f"   ❌ Candidate added notification failed (expected - no candidate object): {str(e)}"
        )
    ...
    try:
        job_notification = NotificationService.send_job_created_notification(
            job=None, recipient=recruiter_user  # We don't have a real job object
        )
        if job_notification:
        ...
        print(
            f"   ❌ Job created notification failed (expected - no job object): {str(e)}"
        )
    ...
    try:
        interview_notification = (
            NotificationService.send_interview_scheduled_notification(
        ...
        print(
            f"   ❌ Interview scheduled notification failed (expected - no interview object): {str(e)}"
        )
    ...
    try:
        reminder_notification = (
            NotificationService.send_interview_reminder_notification(
                interview=None  # We don't have a real interview object
            )
        )
        ...
        print(
            f"   ❌ Interview reminder notification failed (expected - no interview object): {str(e)}"
        )
