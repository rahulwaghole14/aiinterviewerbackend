...
def test_hiring_agency_permissions(token, user_type):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(
                f"✅ {user_type} can list hiring agencies ({len(hiring_agencies)} found)"
            )
        else:
    ...
    try:
        response = requests.post(
            f"{BASE_URL}/hiring_agency/", json=hiring_agency_data, headers=headers
        )
        ...
        if response.status_code == 201:
            ...
            print(
                f"✅ {user_type} correctly blocked from creating hiring agency users (403 Forbidden)"
            )
            return None
        else:
...
def test_hiring_agency_edit_permissions(token, user_id, user_type):
    ...
    try:
        response = requests.put(
            f"{BASE_URL}/hiring_agency/{user_id}/", json=update_data, headers=headers
        )
        ...
        if response.status_code == 200:
            ...
            print(
                f"✅ {user_type} correctly blocked from editing hiring agency users (403 Forbidden)"
            )
        else:
            print(f"❌ Unexpected response for {user_type}: {response.status_code}")
    except Exception as e:
...
def test_recruiter_permissions(token, user_type):
    ...
    try:
        response = requests.post(
            f"{BASE_URL}/companies/recruiters/create/",
            json=recruiter_data,
            headers=headers,
        )
        ...
        if response.status_code == 201:
            ...
            print(
                f"✅ {user_type} correctly blocked from creating recruiters (403 Forbidden)"
            )
            return None
        else:
            print(f"❌ Unexpected response for {user_type}: {response.status_code}")
...
def test_recruiter_edit_permissions(token, recruiter_id, user_type):
    ...
    update_data = {"full_name": f"Updated Recruiter {user_type}", "is_active": True}

    try:
        response = requests.put(
            f"{BASE_URL}/companies/recruiters/{recruiter_id}/",
            json=update_data,
            headers=headers,
        )
        print(
            f"📤 PUT /companies/recruiters/{recruiter_id}/ - Status: {response.status_code}"
        )
        ...
        if response.status_code == 200:
            ...
            print(
                f"✅ {user_type} correctly blocked from editing recruiters (403 Forbidden)"
            )
        else:
            print(f"❌ Unexpected response for {user_type}: {response.status_code}")
    except Exception as e:
...
def test_recruiter_delete_permissions(token, recruiter_id, user_type):
    ...
    try:
        response = requests.delete(
            f"{BASE_URL}/companies/recruiters/{recruiter_id}/", headers=headers
        )
        print(
            f"📤 DELETE /companies/recruiters/{recruiter_id}/ - Status: {response.status_code}"
        )
        ...
        if response.status_code == 204:
            ...
            print(
                f"✅ {user_type} correctly blocked from deleting recruiters (403 Forbidden)"
            )
        else:
            print(f"❌ Unexpected response for {user_type}: {response.status_code}")
    except Exception as e:
...
if __name__ == "__main__":
    ...
    if company_token:
        ...
        if hiring_user:
            test_hiring_agency_edit_permissions(
                company_token, hiring_user["id"], "COMPANY"
            )
    ...
    if admin_token:
        ...
        if admin_hiring_user:
            test_hiring_agency_edit_permissions(
                admin_token, admin_hiring_user["id"], "ADMIN"
            )
        ...
        if admin_recruiter:
            ...
            test_recruiter_delete_permissions(
                admin_token, admin_recruiter["id"], "ADMIN"
            )
