...
class DashboardAnalytics:
    ...
    def get_user_dashboard_data(user, days=30):
        ...
        resume_stats = DashboardAnalytics._get_resume_stats(user, start_date, end_date, base_filters)
        
        # Interview statistics
        from interviews.models import Interview
        interview_stats = DashboardAnalytics._get_interview_stats(user, start_date, end_date, base_filters)
        
        # Candidate statistics
        from candidates.models import Candidate
        candidate_stats = DashboardAnalytics._get_candidate_stats(user, start_date, end_date, base_filters)
        
        # Job statistics
        from jobs.models import Job
        job_stats = DashboardAnalytics._get_job_stats(user, start_date, end_date, base_filters)
    ...
    def _get_resume_stats(user, start_date, end_date, base_filters):
        ...
        successful_uploads = queryset.count()  # All uploaded resumes are considered successful
        failed_uploads = 0  # No status field, so no failed uploads
        
        # Daily upload trend
        daily_uploads = queryset.extra(
            select={'date': 'DATE(resumes_resume.uploaded_at)'}
        ...
        return {
            ...
            'recent_uploads': list(queryset.order_by('-uploaded_at')[:5].values(
                'id', 'file', 'uploaded_at'
            ))
        }
    ...
    def _get_interview_stats(user, start_date, end_date, base_filters):
        ...
        cancelled_interviews = queryset.filter(status='error').count()  # Using 'error' as cancelled
        
        # Daily interview trend
        daily_interviews = queryset.extra(
            select={'date': 'DATE(interviews_interview.created_at)'}
        ).values('date').annotate(
            count=Count('id')
        ).order_by('date')
        ...
        return {
            ...
            'upcoming_interviews': list(queryset.filter(
                status='scheduled',
                started_at__gte=timezone.now()
            ).order_by('started_at')[:5].values(
                'id', 'candidate__full_name', 'started_at', 'status'
            ))
        }
    ...
    def _get_candidate_stats(user, start_date, end_date, base_filters):
        ...
        # Domain distribution
        domain_distribution = queryset.values('domain').annotate(
            count=Count('id')
        ).order_by('-count')[:10]
        ...
        return {
            ...
            'recent_candidates': list(queryset.order_by('-created_at')[:5].values(
    ...
    def _get_job_stats(user, start_date, end_date, base_filters):
        ...
        level_distribution = queryset.values('position_level').annotate(
            count=Count('id')
        ).order_by('-count')
        ...
        return {
            ...
            'recent_jobs': list(queryset.order_by('-created_at')[:5].values(
    ...
    def _get_activity_data(user, start_date, end_date):
        ...
        daily_activity = queryset.extra(
            select={'date': 'DATE(dashboard_useractivity.created_at)'}
        ).values('date').annotate(
            count=Count('id')
        ).order_by('date')
    ...
    def _get_performance_data(start_date, end_date):
        ...
        for metric in queryset.values('metric_name').distinct():
            ...
            latest = queryset.filter(metric_name=metric_name).order_by('-timestamp').first()
            if latest:
        ...
        return {
            ...
            'performance_trend': list(queryset.order_by('-timestamp')[:20].values(
