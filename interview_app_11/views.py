...
try:
    ...
    print("✅ Google Cloud Text-to-Speech imported successfully in interview_app_11/views.py")
except ImportError as e:
    print(f"❌ Warning: google-cloud-texttospeech not available in interview_app_11/views.py: {e}")
    texttospeech = None
except Exception as e:
    print(f"❌ Unexpected error importing google-cloud-texttospeech in interview_app_11/views.py: {e}")
    texttospeech = None
...
# Placeholder VideoCamera class since camera modules are not available
class VideoCamera:
    def __init__(self, session_id=None):
        self.session_id = session_id
        self.video = type('Video', (), {'isOpened': lambda: False})()
    
    def get_frame(self):
        # Return a placeholder frame (empty bytes)
        return b''
    ...
    try:
        ...
        if False:  # Disable existing questions to force new AI generation
            ...
            for q in session.questions.filter(question_type='CODING').order_by('order'):
                # Get job information for dynamic question generation
                job_title = None
                domain_name = None
                ...
                # Generate dynamic coding questions based on job profile using Gemini API
                from .gemini_question_generator import get_coding_questions_from_gemini
                dynamic_questions = get_coding_questions_from_gemini(job_title, domain_name)
                
                # Use the first dynamic question or fallback to default
                if dynamic_questions:
                    coding_q = dynamic_questions[0].copy()
                    coding_q['id'] = str(q.id)
                    coding_q['description'] = q.question_text
                else:
            ...
            if 'generate_new_questions' in locals() and generate_new_questions:
                ...
                if DEV_MODE:
                    ...
                    # Add dynamic coding questions based on job profile
                    job_title = None
                    domain_name = None
                    if hasattr(session, 'interview') and session.interview and session.interview.job:
                        job_title = session.interview.job.job_title
            ...
            if DEV_MODE:
                ...
                master_prompt = (
                    ...
                    "Then, generate a mix of technical Questions. "
...
def end_interview_session(request):
    try:
        ...
        if session.status == 'SCHEDULED':
            ...
            try:
                from .comprehensive_evaluation_service import comprehensive_evaluation_service
...
def execute_python_windows(code, test_input):
    ...
    if function_match:
        ...
        # Handle pandas test cases specially
        if 'pd.DataFrame' in test_input or 'pandas' in code.lower():
            # For pandas questions, we need to import pandas and create the test data
            full_script = f"""
import pandas as pd
{code}
...
# Test case
{test_input}
result = {function_name}(transactions)
print(result)
"""
        else:
            # Regular test case
...
def get_test_case_for_question(question):
    """
    Get appropriate test case for a coding question based on job profile
    ...
    job_title = None
    domain_name = None
    if hasattr(question, 'session') and question.session and hasattr(question.session, 'interview'):
        if question.session.interview and question.session.interview.job:
            job_title = question.session.interview.job.job_title
            if question.session.interview.job.domain:
                domain_name = question.session.interview.job.domain.name
    
    # Get dynamic questions based on job profile using Gemini API
    from .gemini_question_generator import get_coding_questions_from_gemini
    dynamic_questions = get_coding_questions_from_gemini(job_title, domain_name)
    
    # Find matching question by description or use first available
    matching_question = None
    if dynamic_questions:
        # Try to match by description
        for dq in dynamic_questions:
            if dq['description'] in question.question_text or question.question_text in dq['description']:
                matching_question = dq
                break
        
        # If no match found, use first question
        if not matching_question:
            matching_question = dynamic_questions[0]
    
    # Return test case from matching question
    if matching_question and matching_question.get('test_cases'):
        test_case = matching_question['test_cases'][0]  # Use first test case
        return test_case['input'], test_case['output']
    
    # Fallback to default reverse string test case
...
def run_test_suite(code, language, test_cases, question_description=None):
    """
    Runs the given code against a set of test cases for a specific language.
    Uses Gemini API for enhanced evaluation.
    Returns a tuple: (all_passed, output_log_string, gemini_evaluation)
    """
    if not test_cases:
        return False, "No test cases found for this question.", None
    ...
    if not execution_function:
        return False, f"Language '{language}' is not supported.", None
    ...
    # Use Gemini API for enhanced evaluation if question description is provided
    gemini_evaluation = None
    if question_description:
        try:
            from .gemini_question_generator import evaluate_code_with_gemini
            gemini_evaluation = evaluate_code_with_gemini(code, question_description, test_results, language)
        except Exception as e:
            print(f"Error in Gemini evaluation: {e}")
            gemini_evaluation = None
    
    return all_passed, "\n".join(output_log_lines), gemini_evaluation
@csrf_exempt
@require_POST
...
def submit_coding_challenge(request):
    try:
        ...
        # Run enhanced validation with Gemini API evaluation
        print("Running enhanced validation with Gemini API evaluation")
        
        # Get test cases based on question type and job profile
        test_input, expected_output = get_test_case_for_question(question)
        
        if not test_input or not expected_output:
            return JsonResponse({"status": "error", "message": "No test cases available for this question."}, status=400)
        
        # Create test case objects for the enhanced test suite
        ...
        # Run enhanced test suite with Gemini evaluation
        passed_all, final_log, gemini_evaluation = run_test_suite(
            submitted_code, 
            language, 
            test_cases, 
        ...
        # Store Gemini evaluation results if available
        if gemini_evaluation:
            submission.gemini_evaluation = json.dumps(gemini_evaluation)
            submission.save()
            print(f"Gemini evaluation stored: {gemini_evaluation.get('overall_score', 'N/A')}/100")
