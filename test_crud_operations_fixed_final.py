#!/usr/bin/env python3
"""
Final CRUD Operations Test - All Issues Fixed
...
- Hiring Agency Detail URL: Fixed URL routing
- Hiring Agency Update: Added partial update support
- Candidate Update: Created separate update serializer
- Recruiter Create: Made fields optional and flexible
"""
...
class CRUDTester:
    ...
    def test_company_crud(self) -> Dict[str, Any]:
        ...
        try:
            response = self.session.post(
                f"{BASE_URL}/api/companies/", json=company_data
            )
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"❌ Companies list failed: {response.status_code} - {response.text}"
                )
        except Exception as e:
        ...
        if self.test_data.get("company_id"):
            ...
            try:
                response = self.session.get(
                    f"{BASE_URL}/api/companies/{self.test_data['company_id']}/"
                )
                if response.status_code == 200:
                    ...
                    print(
                        f"❌ Company detail failed: {response.status_code} - {response.text}"
                    )
            except Exception as e:
        ...
        if self.test_data.get("company_id"):
            ...
            try:
                ...
                if response.status_code == 204:
                    ...
                    print(
                        f"❌ Company deletion failed: {response.status_code} - {response.text}"
                    )
            except Exception as e:
    ...
    def test_hiring_agency_crud(self) -> Dict[str, Any]:
        ...
        try:
            response = self.session.post(
                f"{BASE_URL}/api/hiring_agency/add_user/", json=agency_data
            )
            if response.status_code == 201:
                ...
                print(
                    f"❌ Hiring Agency creation failed: {response.status_code} - {response.text}"
                )
        except Exception as e:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"❌ Hiring Agencies list failed: {response.status_code} - {response.text}"
                )
        except Exception as e:
            print(f"❌ Hiring Agencies list error: {str(e)}")

        # READ (Detail) - Fixed URL
        if self.test_data.get("agency_id"):
            ...
            try:
                response = self.session.get(
                    f"{BASE_URL}/api/hiring_agency/{self.test_data['agency_id']}/"
                )
                if response.status_code == 200:
                    agency_detail = response.json()
                    print(
                        f"✅ Hiring Agency detail retrieved: {agency_detail['first_name']} {agency_detail['last_name']}"
                    )
                else:
                    print(
                        f"❌ Hiring Agency detail failed: {response.status_code} - {response.text}"
                    )
            except Exception as e:
                print(f"❌ Hiring Agency detail error: {str(e)}")

        # UPDATE - Fixed partial update
        if self.test_data.get("agency_id"):
            print(
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    print(
                        f"✅ Hiring Agency updated successfully: {updated_agency['first_name']} {updated_agency['last_name']}"
                    )
                    results["update"] = True
                else:
                    print(
                        f"❌ Hiring Agency update failed: {response.status_code} - {response.text}"
                    )
            except Exception as e:
        ...
        if self.test_data.get("agency_id"):
            ...
            try:
                ...
                if response.status_code == 204:
                    ...
                    print(
                        f"❌ Hiring Agency deletion failed: {response.status_code} - {response.text}"
                    )
            except Exception as e:
    ...
    def test_recruiter_crud(self) -> Dict[str, Any]:
        ...
        # CREATE - Fixed field requirements
        print("\n📝 Testing Recruiter CREATE...")
        recruiter_data = {
            "email": f"testrecruiter{int(time.time())}@example.com",
            "full_name": f"Test Recruiter {int(time.time())}",
        ...
        try:
            response = self.session.post(
                f"{BASE_URL}/api/companies/recruiters/create/", json=recruiter_data
            )
            if response.status_code == 201:
                ...
                print(
                    f"❌ Recruiter creation failed: {response.status_code} - {response.text}"
                )
        except Exception as e:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"❌ Recruiters list failed: {response.status_code} - {response.text}"
                )
        except Exception as e:
        ...
        if self.test_data.get("recruiter_id"):
            ...
            try:
                response = self.session.get(
                    f"{BASE_URL}/api/companies/recruiters/{self.test_data['recruiter_id']}/"
                )
                if response.status_code == 200:
                    recruiter_detail = response.json()
                    print(
                        f"✅ Recruiter detail retrieved: {recruiter_detail['full_name']}"
                    )
                else:
                    print(
                        f"❌ Recruiter detail failed: {response.status_code} - {response.text}"
                    )
            except Exception as e:
        ...
        if self.test_data.get("recruiter_id"):
            ...
            update_data = {
                "full_name": f"Updated Recruiter {int(time.time())}",
                "is_active": True,
            }
            try:
                response = self.session.put(
                ...
                if response.status_code == 200:
                    ...
                    print(
                        f"✅ Recruiter updated successfully: {updated_recruiter['full_name']}"
                    )
                    results["update"] = True
                else:
                    print(
                        f"❌ Recruiter update failed: {response.status_code} - {response.text}"
                    )
            except Exception as e:
        ...
        if self.test_data.get("recruiter_id"):
            ...
            try:
                ...
                if response.status_code == 204:
                    ...
                    print(
                        f"❌ Recruiter deletion failed: {response.status_code} - {response.text}"
                    )
            except Exception as e:
    ...
    def test_candidate_crud(self) -> Dict[str, Any]:
        ...
        try:
            with open(test_resume_file, "rb") as f:
                ...
                response = self.session.post(
                    f"{BASE_URL}/api/candidates/", data=data, files=files
                )
                if response.status_code == 201:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"❌ Candidates list failed: {response.status_code} - {response.text}"
                )
        except Exception as e:
        ...
        if self.test_data.get("candidate_id"):
            ...
            try:
                response = self.session.get(
                    f"{BASE_URL}/api/candidates/{self.test_data['candidate_id']}/"
                )
                if response.status_code == 200:
                    candidate_detail = response.json()
                    print(
                        f"✅ Candidate detail retrieved: {candidate_detail['full_name']}"
                    )
                else:
                    print(
                        f"❌ Candidate detail failed: {response.status_code} - {response.text}"
                    )
            except Exception as e:
                print(f"❌ Candidate detail error: {str(e)}")

        # UPDATE - Fixed to not require resume_file
        if self.test_data.get("candidate_id"):
            print(
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    print(
                        f"✅ Candidate updated successfully: {updated_candidate['full_name']}"
                    )
                    results["update"] = True
                else:
                    print(
                        f"❌ Candidate update failed: {response.status_code} - {response.text}"
                    )
            except Exception as e:
        ...
        if self.test_data.get("candidate_id"):
            ...
            try:
                ...
                if response.status_code == 204:
                    ...
                    print(
                        f"❌ Candidate deletion failed: {response.status_code} - {response.text}"
                    )
            except Exception as e:
    ...
    def run_all_tests(self):
        """Run all CRUD tests"""
        print("🚀 STARTING FINAL CRUD OPERATIONS TEST - ALL ISSUES FIXED")
        print("=" * 80)
        ...
        # Summary
        print("\n" + "=" * 80)
        print("📊 FINAL CRUD OPERATIONS TEST SUMMARY")
        print("=" * 80)
        ...
        if successful_operations == total_operations:
            print("\n🎉 ALL CRUD OPERATIONS PASSED! ALL ISSUES FIXED!")
        else:
            print(
                f"\n⚠️ {total_operations - successful_operations} operations still failed"
            )
