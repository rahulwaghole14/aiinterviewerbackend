...
class DataExtractionSerializer(serializers.Serializer):
    ...
    resume_file = serializers.FileField(
        required=True, help_text="Resume file (PDF, DOCX, DOC)"
    )
    domain = serializers.CharField(
        max_length=100, required=True, help_text="Domain/technology area"
    )
    role = serializers.CharField(
        max_length=100, required=True, help_text="Job role/position"
    )
...
class BulkCandidateSubmissionSerializer(serializers.Serializer):
    ...
    role = serializers.CharField(
        max_length=100, required=True, help_text="Job role/position for all candidates"
    )
        ...
        for candidate in value:
            if not candidate.get("filename"):
                raise serializers.ValidationError(
                    "Each candidate must have a filename."
                )
...
class BulkCandidateCreationSerializer(serializers.Serializer):
    ...
    resume_files = serializers.ListField(
        child=serializers.FileField(help_text="Resume file (PDF, DOCX, DOC)"),
        min_length=1,
...
class CandidateUpdateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            "domain",
            "poc_email",
            "status",  # Added status field for hire/reject functionality
        ]
        read_only_fields = ["id"]
