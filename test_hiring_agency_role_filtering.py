        ...
        if response.status_code == 200:
            ...
            if non_hiring_agencies:
                ...
                for item in non_hiring_agencies:
                    print(
                        f"   - ID {item['id']}: {item['email']} (Role: {item['role']}, Company: {item['company_name']})"
                    )
                return False
            else:
    ...
    try:
        ...
        if admin_response.status_code == 200:
            ...
            for company_name, agencies in companies.items():
                hiring_agency_count = sum(
                    1 for agency in agencies if agency.get("role") == "Hiring Agency"
                )
                other_roles_count = len(agencies) - hiring_agency_count
                print(
                    f"   - {company_name}: {hiring_agency_count} hiring agencies, {other_roles_count} other roles"
                )
