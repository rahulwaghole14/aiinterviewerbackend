...
def test_evaluation_summary(token, interview_id):
    ...
    if response.status_code == 200:
        ...
        log(f"⚠️  No evaluation found for interview {interview_id} (this is normal if no evaluation exists)")
    else:
        log(f"❌ Evaluation summary failed: {response.status_code} - {response.text}", "ERROR")
...
def test_evaluation_report(token, interview_id):
    ...
    if response.status_code == 200:
        ...
        log(f"⚠️  No evaluation report found for interview {interview_id} (this is normal if no evaluation exists)")
    else:
        log(f"❌ Evaluation report failed: {response.status_code} - {response.text}", "ERROR")
...
def test_submit_feedback(token, candidate_id, interview_id):
    ...
    if response.status_code == 201:
        ...
        log(f"❌ Feedback submission failed: {response.status_code} - {response.text}", "ERROR")
        return None
...
def test_all_feedbacks(token, candidate_id):
    ...
    if response.status_code == 200:
        ...
        log(f"❌ All feedbacks failed: {response.status_code} - {response.text}", "ERROR")
...
def test_evaluation_list(token):
    ...
    if response.status_code == 200:
        ...
        log(f"✅ Evaluation list retrieved successfully: {len(evaluations)} evaluations")
        ...
        log(f"❌ Evaluation list failed: {response.status_code} - {response.text}", "ERROR")
