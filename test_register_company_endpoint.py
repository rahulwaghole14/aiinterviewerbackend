    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for i, company in enumerate(companies, 1):
                print(
                    f"   {i}. ID: {company.get('id')} | Name: {company.get('name')} | Active: {company.get('is_active')}"
                )
...
def test_company_creation_via_api():
    ...
    try:
        response = requests.post(
            f"{BASE_URL}/api/companies/", json=new_company_data, headers=headers
        )
...
def main():
    ...
    print("=" * 60)
    print(
        f"✅ Companies registered via /api/auth/register/: {len(registered_companies)}"
    )
    print(
        f"✅ Companies found in /api/companies/: {len(all_companies) if all_companies else 0}"
    )
    print(
        f"✅ Company created via /api/companies/: {'Yes' if api_created_company else 'No'}"
    )
    ...
    if registered_companies and all_companies:
        ...
        registered_company_names = [
            comp["company_name"] for comp in registered_companies
        ]
        ...
        if missing_companies:
            ...
            print(
                "   This suggests the registration process is not creating Company model entries."
            )
        else:
            print(
