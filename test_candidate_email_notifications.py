...
def test_interview_creation_with_notification():
    ...
    if response.status_code == 200:
        ...
        interview_notifications = [
            ...
            if "interview" in n.get("notification_type", "").lower()
        ]

        if interview_notifications:
            print(
                f"✅ Found {len(interview_notifications)} interview notifications in system"
            )
            ...
            print(
                "⚠️  No interview notifications found in system (email may have been sent directly)"
            )
    else:
...
def test_interview_scheduling_with_notification():
    ...
    response = requests.post(
        f"{BASE_URL}/api/interviews/slots/", json=slot_data, headers=headers
    )
    if response.status_code == 201:
        ...
        print(
            f"❌ AI Interview Slot creation failed: {response.status_code} - {response.text}"
        )
        return False
    ...
    response = requests.post(
        f"{BASE_URL}/api/interviews/schedules/book_interview/",
        json=booking_data,
        headers=headers,
    )
    if response.status_code == 201:
...
def test_email_configuration():
    ...
    if response.status_code == 200:
        ...
        if interview_template:
            print(
                f"✅ Found interview scheduled template: {interview_template.get('name')}"
            )
...
def main():
    ...
    print("\n📋 Summary:")
    print(
        "1. Email configuration test - Checks if email settings are properly configured"
    )
    print(
        "2. Interview creation notification test - Tests email when interviews are created"
    )
    print(
        "3. Interview scheduling notification test - Tests email when interviews are scheduled with slots"
    )
    print(
        "4. Slot booking notification test - Tests email when slots are booked directly"
    )
    ...
    print(
        "   The test emails should be sent to the candidate email addresses shown above."
    )
