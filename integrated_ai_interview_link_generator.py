...
from ai_interview.models import AIInterviewSession, AIInterviewQuestion, AIInterviewResponse, AIInterviewResult
...
class IntegratedAIInterviewLinkGenerator:
    ...
    def create_ai_interview_session(self, interview):
        ...
        try:
            ...
            ai_configuration = {
                ...
                'resume_text': getattr(interview.candidate.resume, 'extracted_text', '') if interview.candidate.resume else 'No resume available',
    ...
    def create_interview_session(self, interview, link_token):
        ...
        try:
            ...
            if interview.candidate.resume:
                resume_text = getattr(interview.candidate.resume, 'extracted_text', resume_text)
    ...
    def test_api_endpoints(self, interview, link_token):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.print_info(f"Candidate: {interview_data.get('candidate_name', 'Unknown')}")
                self.print_info(f"Job: {interview_data.get('job_title', 'Unknown')}")
            else:
                self.print_warning(f"Public interview access returned {response.status_code}")
            ...
            if response.status_code == 200:
                ...
                self.print_info(f"Session ID: {session_data.get('session_id', 'Unknown')}")
            else:
                self.print_warning(f"AI interview start returned {response.status_code}")
    ...
    def generate_interview_link(self, candidate_name="AKSHAY PATIL"):
        ...
        try:
            self.print_step(1, "SETUP", "Initializing integrated AI interview link generator")
            print(f"🎯 Target Candidate: {candidate_name}")
            print(f"🌐 Base URL: {self.base_url}")
            print(f"⏰ Current Time: {datetime.now(self.ist).strftime('%Y-%m-%d %H:%M:%S IST')}")
            ...
            self.print_step(9, "PORTAL SESSION", "Creating interview session for portal")
            interview_session = self.create_interview_session(interview, interview.interview_link)
            ...
            print(f"⏰ Valid Until: {interview.link_expires_at.strftime('%Y-%m-%d %H:%M:%S IST')}")
            print("="*60)
