...
def test_interview_scheduling_notification():
    ...
    if response.status_code == 200:
        ...
        scheduling_notifications = [
            n
            for n in notifications
            if "scheduled" in n.get("notification_type", "").lower()
        ]
...
def test_candidate_email_notification():
    ...
    if response.status_code == 200:
        ...
        if interview_template:
            ...
            print(
                f"   Title template: {interview_template.get('title_template', 'N/A')}"
            )
            print(
                f"   Message template: {interview_template.get('message_template', 'N/A')}"
            )
...
def main():
    ...
    print("\n📋 Summary:")
    print(
        "1. Interview creation notification test - Checks if notifications are sent when interviews are created"
    )
    print(
        "2. Interview scheduling notification test - Checks if notifications are sent when interviews are scheduled with slots"
    )
    print(
        "3. Candidate email notification test - Checks if notification templates support candidate emails"
    )
