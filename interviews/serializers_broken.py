...
class InterviewSerializer(serializers.ModelSerializer):
    ...
    def get_questions_and_answers(self, obj):
        ...
        try:
            ...
            # Use the new QAConversationPair service to get chronological Q&A pairs
            qa_data = get_qa_pairs_for_session_ordered(session.session_key)
            chronological_pairs = qa_data['chronological_pairs']
            
            print(f"✅ Found {chronological_pairs.count()} Q&A pairs for session {session.session_key}")
...
class InterviewConflictSerializer(serializers.ModelSerializer):
    ...
    def get_screen_recording_file(self, obj):
        """Get screen recording file URL from InterviewSession if available"""
        try:
            from interview_app.models import InterviewSession
            from django.conf import settings
            
            # First check if the file is directly on the Interview model
            ...
            # If not, find the session for this interview using session_key
            session = None
            if obj.session_key:
                try:
                    session = InterviewSession.objects.get(session_key=obj.session_key)
            ...
            # Return video URL if session exists and has screen recording
            if session:
                if session.screen_recording_gcs_url:
                    return session.screen_recording_gcs_url
                if session.screen_recording:
                    request = self.context.get('request')
                    if request:
                        return request.build_absolute_uri(session.screen_recording.url)
                    return session.screen_recording.url
    ...
    def get_screen_recording_url(self, obj):
        """Get screen recording URL - check model field, then session gcs_url, then local file"""
        try:
            if obj.screen_recording_url:
                return obj.screen_recording_url
            
            # Use get_screen_recording_file logic but prioritize GCS if available
            # Since get_screen_recording_file already prioritized GCS in my implementation above, 
            # we can just call it or re-implement similar logic. 
            # For simplicity, let's re-implement to be explicit about GCS priority if desired.
            ...
            # Fallback to model file if session lookup failed
            if obj.screen_recording_file:
                request = self.context.get('request')
                if request:
                    return request.build_absolute_uri(obj.screen_recording_file.url)
