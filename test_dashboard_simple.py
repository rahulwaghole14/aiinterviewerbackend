...
from dashboard.models import DashboardAnalytics, DashboardMetric, UserActivity, DashboardWidget, SystemPerformance
...
def test_dashboard_system():
    ...
    try:
        ...
        resume_stats = DashboardAnalytics._get_resume_stats(admin_user, start_date, end_date, {})
        log(f"✓ Resume statistics: {resume_stats}")
        
        interview_stats = DashboardAnalytics._get_interview_stats(admin_user, start_date, end_date, {})
        log(f"✓ Interview statistics: {interview_stats}")
        
        candidate_stats = DashboardAnalytics._get_candidate_stats(admin_user, start_date, end_date, {})
        log(f"✓ Candidate statistics: {candidate_stats}")
        
        job_stats = DashboardAnalytics._get_job_stats(admin_user, start_date, end_date, {})
        log(f"✓ Job statistics: {job_stats}")
        
        activity_data = DashboardAnalytics._get_activity_data(admin_user, start_date, end_date)
    ...
    for user, role in [(admin_user, "ADMIN"), (company_user, "COMPANY"), (agency_user, "HIRING_AGENCY")]:
        try:
