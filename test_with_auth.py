    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for i, res in enumerate(result.get("results", []), 1):
                if res.get("success"):
                    print(
                        f"  {i}. ✅ {res.get('filename')} - {res.get('extracted_data', {})}"
                    )
                else:
                    print(
                        f"  {i}. ❌ {res.get('filename')} - {res.get('error_message', 'Unknown error')}"
                    )
        else:
...
def test_single_upload_with_real_auth(token):
    ...
    files = {"file": ("alice_johnson.pdf", mock_resume_content, "application/pdf")}
    ...
    try:
        ...
        if response.status_code == 201:
            ...
            print(
                f"📝 Parsed text length: {len(result.get('parsed_text', ''))} characters"
            )
        else:
