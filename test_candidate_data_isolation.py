...
def test_candidate_data_isolation():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"   📊 Candidates grouped by {len(candidates_by_recruiter)} recruiters")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"   ✅ Admin info: {admin_info.get('email')} - {admin_info.get('role')}")
        else:
        ...
        if agency_candidates:
            ...
            for other_agency in agencies_with_candidates:
                if other_agency.get('id') != agency_id:
                    other_agency_candidates = candidates_by_recruiter.get(other_agency.get('id'), [])
                    print(f"   - Other agency {other_agency.get('email')}: {len(other_agency_candidates)} candidates")
        else:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            candidates_without_recruiter = [c for c in candidates if not c.get('recruiter')]
