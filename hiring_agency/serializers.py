...
class UserDataSerializer(serializers.ModelSerializer):
    ...
    input_company_name = serializers.CharField(
    ...
    def validate_name(self, value, field_name):
        if len(value.strip()) < 2:
            raise serializers.ValidationError(
                f"{field_name} must be at least 2 characters long."
            )
        return value
    ...
    def validate_permission_granted(self, value):
        if value > date.today():
            raise serializers.ValidationError(
                "Permission granted date cannot be in the future."
            )
        return value
    ...
    def validate(self, data):
        ...
        if new_role in ["RECRUITER", "HIRING_AGENCY"]:
