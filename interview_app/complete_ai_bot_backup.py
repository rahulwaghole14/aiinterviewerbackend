...
#         "can you elaborate", "please elaborate", "say it "
#     ]
...
# # ------------------ Centralized Question Generation with Strict Limit Enforcement ------------------
...
#         print(f"🔍 upload_answer START: session_id={session_id}")
#         print(f"🔍 Current state: question_number={session.current_question_number}, max_questions={session.max_questions}, completed={session.is_completed}")
#         print(f"🔍 Transcript: '{transcript[:50]}...'" if len(transcript) > 50 else f"'{transcript}'")
...
#                 question_type = "follow_up" if should_follow_up else "regular"
                
#                 # Use centralized question generation with limit check
#                 next_q_text = generate_next_question_with_limit_check(
#                     session, 
#                     question_type, 
#                     last_answer_text=transcript, 
#                     context="PROCEED FLOW"
#                 )
...
#             session.regular_questions_count += 1
#             # Use question directly without any reassuring phrases - be professional like a real technical interviewer
...
#         # STRICT QUESTION COUNT ENFORCEMENT - Primary check before any question generation
#         print(f"🔍 QUESTION COUNT CHECK: Before generating - current={session.current_question_number}, max={session.max_questions}")
#         if session.current_question_number >= session.max_questions:
#             print(f"🛑 QUESTION LIMIT REACHED: current={session.current_question_number}, max={session.max_questions}")
#             print(f"🛑 STOPPING question generation - interview should be completed")
...
#         # STRICT QUESTION COUNT ENFORCEMENT - Check if we've reached the limit before generating any new questions
#         if session.current_question_number >= session.max_questions:
#             print(f"🛑 QUESTION LIMIT REACHED: current={session.current_question_number}, max={session.max_questions}")
#             print(f"🛑 STOPPING question generation - interview should be completed")
#             # Mark interview as completed
...
#         if should_follow_up:
#             session.follow_up_questions_count += 1
#             print(f"🤖 Generating follow-up question {session.current_question_number}/{session.max_questions} based on answer snippet:", (transcript[:120] + ('...' if len(transcript) > 120 else '')))
#         else:
#             session.regular_questions_count += 1
#             print(f"🤖 Generating regular question {session.current_question_number}/{session.max_questions} based on answer snippet:", (transcript[:120] + ('...' if len(transcript) > 120 else '')))
        
#         # Show follow-up ratio
