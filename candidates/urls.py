...
urlpatterns = [
    ...
    path(
        "bulk-create/",
        BulkCandidateCreationView.as_view(),
        name="bulk-candidate-creation",
    ),
    # Step-by-step candidate creation endpoints
    path(
        "select-domain/",
        DomainRoleSelectionView.as_view(),
        name="domain-role-selection",
    ),
    path("extract-data/", DataExtractionView.as_view(), name="data-extraction"),
    path(
        "verify/<int:draft_id>/",
        CandidateVerificationView.as_view(),
        name="candidate-verification",
    ),
    path(
        "submit/<int:draft_id>/",
        CandidateSubmissionView.as_view(),
