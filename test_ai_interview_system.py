...
def test_ai_interview_slot_creation():
    ...
    response = requests.post(
        f"{BASE_URL}/api/companies/", json=company_data, headers=headers
    )
...
def test_ai_interview_configuration():
    ...
    response = requests.post(
        f"{BASE_URL}/api/interviews/configurations/", json=config_data, headers=headers
    )
    if response.status_code == 201:
        ...
        print(
            f"❌ AI Interview Configuration creation failed: {response.status_code} - {response.text}"
        )
        return False
...
def test_recurring_ai_slots():
    ...
    if response.status_code == 201:
        ...
        print(
            f"❌ Recurring AI Slots creation failed: {response.status_code} - {response.text}"
        )
        return False
...
def test_ai_interview_scheduling():
    ...
    response = requests.get(
        f"{BASE_URL}/api/interviews/available-slots/?start_date={datetime.now().date().isoformat()}",
        headers=headers,
    )
    if response.status_code == 200:
        slots = response.json()
        if slots:
            ...
            if response.status_code == 201:
                ...
                print(
                    f"❌ Interview booking failed: {response.status_code} - {response.text}"
                )
                return False
        else:
