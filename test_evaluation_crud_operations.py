...
def test_evaluation_crud(token, interview_id):
    ...
    response = requests.post(
        f"{BASE_URL}/api/evaluation/crud/", json=evaluation_data, headers=headers
    )
    if response.status_code == 201:
    ...
    if response.status_code == 200:
        ...
        log(
            f"❌ Evaluation list retrieval failed: {response.status_code} - {response.text}",
            "ERROR",
        )
    ...
    response = requests.get(
        f"{BASE_URL}/api/evaluation/crud/{evaluation_id}/", headers=headers
    )
    if response.status_code == 200:
        ...
        log(
            f"❌ Evaluation detail retrieval failed: {response.status_code} - {response.text}",
            "ERROR",
        )
    ...
    response = requests.put(
        f"{BASE_URL}/api/evaluation/crud/{evaluation_id}/",
        json=update_data,
        headers=headers,
    )
    if response.status_code == 200:
        ...
        log(
            f"❌ Evaluation update failed: {response.status_code} - {response.text}",
            "ERROR",
        )
    ...
    if response.status_code == 204:
        ...
        log(
            f"❌ Evaluation deletion failed: {response.status_code} - {response.text}",
            "ERROR",
        )
...
def test_evaluation_validation(token, interview_id):
    ...
    response = requests.post(
        f"{BASE_URL}/api/evaluation/crud/", json=invalid_data, headers=headers
    )
    ...
    response = requests.post(
        f"{BASE_URL}/api/evaluation/crud/", json=valid_data, headers=headers
    )
    if response.status_code == 201:
        evaluation = response.json()
        evaluation_id = evaluation["id"]
        ...
        if response2.status_code == 400:
            ...
            log(
                f"❌ Duplicate evaluation should have been rejected: {response2.status_code}"
            )
        ...
        log(
            f"❌ Failed to create test evaluation: {response.status_code} - {response.text}",
            "ERROR",
        )
...
def test_legacy_endpoints(token, interview_id):
    ...
    response = requests.get(
        f"{BASE_URL}/api/evaluation/summary/{interview_id}/", headers=headers
    )
    if response.status_code == 200:
        summary = response.json()
        log(
            f"✅ Legacy summary endpoint working: Score {summary.get('overall_score', 'N/A')}"
        )
    else:
        log(
            f"❌ Legacy summary endpoint failed: {response.status_code} - {response.text}",
            "ERROR",
        )
    ...
    if response.status_code == 200:
        ...
        log(
            f"✅ Legacy report endpoint working: Score {report.get('overall_score', 'N/A')}"
        )
    else:
        log(
            f"❌ Legacy report endpoint failed: {response.status_code} - {response.text}",
            "ERROR",
        )
